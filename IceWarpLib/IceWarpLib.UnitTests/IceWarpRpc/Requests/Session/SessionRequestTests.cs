...
namespace IceWarpLib.UnitTests.IceWarpRpc.Requests.Session
{
    ...
    {
        private readonly string _requestsTestDataPath = $@"{TestHelper.GetTestsPath()}\IceWarpRpc\Requests\Session\TestData\Requests";
        private readonly string _responsesTestDataPath = $@"{TestHelper.GetTestsPath()}\IceWarpRpc\Requests\Session\TestData\Responses";
        ...
        public void GetAuthChallenge()
        {
            ...
            Assert.AreEqual(
				"2016-04-12 12:08", 
				response.Timestamp.AddHours(-TimeZoneInfo.Local.GetUtcOffset(DateTime.UtcNow).Hours).ToString("yyyy-MM-dd HH:mm")
			);
        }
